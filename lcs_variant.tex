...
Von einem positiven bzw. negativen Ereignis spricht man, wenn sich der \emph{base reward} im Vergleich zum vorangegangenen Zeitschritt verändert hat, also wenn das Zielobjekt sich in Übertragungsreichweite bzw. aus ihr heraus bewegt hat (siehe Abbildung~\ref{saved_rewards:fig}).\\
...
\caption[Schematische Darstellung der Bewertung von \emph{action set} Listen bei einem neutralen Ereignis] {Schematische Darstellung der Bewertung von \emph{action set} Listen bei einem neutralen Ereignis (mit \emph{base reward} = 0)}
\label{neutral_reward:fig}
\end{figure}


Wählt man den Stack zu groß, dann werden eher am Ergebnis unbeteiligte (d.h. vom Ereignis weit weg liegende) \emph{action set} Listen bewertet. Auch nimmt die Reaktionsgeschwindigkeit des Systems ab, d.h. zum einen bewegen sich die Agenten zu Beginn, bis sie das erste Mal das Zielobjekt in Sichtweite haben, zufällig. Wählt man den Stack zu klein, kann es sein, dass ein Überlauf und somit u.U. ein gewisser Fehler auftritt. Der Wert \emph{maxStackSize} stellt also einen Kompromiss zwischen Zeitverzögerung bzw. Reaktionsgeschwindigkeit und Genauigkeit dar. Wie Abbildung~\ref{max_stack_size:fig} zeigt, ist dies bei größerer Schrittzahl aber vernachlässigbar, die erreichten Qualitäten unterscheiden sich bis zu einer Größe kaum voneinander. 
Dass der \emph{maxStackSize} Wert zwar für einen großen Wertebereich gleich gute Qualität liefert, es aber doch vom Szenario abhängt, zeigt ein Vergleich mit einem 32x32 großen Säulenszenario (siehe Abbildung~\ref{max_stack_size_large:fig}). Hier trifft der Agent deutlich weniger häufig auf das Zielobjekt, es kommt also eher zu einem Überlauf des Stacks.
Auch beim schwierigen Szenario ist die Lage ähnlich. Insbesondere bei relativ wenigen Schritten nimmt der \emph{maxStackSize} Wert aber eine zentrale Rolle ein. Abbildung~\ref{max_stack_size_difficult:fig} zeigt, dass ein Wert von 
TODO 
Insgesamt, scheint ein 
, ein Wert von 128 erscheint sinnvoll.\\

Vergleich prediction error!
Wasted movements
...
\caption[Vergleich verschiedener Werte für \emph{maxStackSize} (Säulenszenario)] {Vergleich verschiedener Werte für \emph{maxStackSize} (Säulenszenario, Zielobjekt mit einfacher Richtungsänderung, Geschwindigkeit 2, SXCS Agenten)}
\label{max_stack_size:fig}
\end{figure}

\begin{figure}[htbp]
\centerline{	
%\includegraphics{max_stack_size_large.eps}
}
\caption[Vergleich verschiedener Werte für \emph{maxStackSize} (Säulenszenario)] {Vergleich verschiedener Werte für \emph{maxStackSize} (Säulenszenario mit 32x32 Feldern, Zielobjekt mit einfacher Richtungsänderung, Geschwindigkeit 2, 8 SXCS Agenten)}
\label{max_stack_size_large:fig}
\end{figure}

\begin{figure}[htbp]
\centerline{	
%\includegraphics{max_stack_size_difficult.eps}
}
\caption[Vergleich verschiedener Werte für \emph{maxStackSize} (schwieriges Szenario)] {Vergleich verschiedener Werte für \emph{maxStackSize} (schwierigen Szenario, Zielobjekt bewegt sich nur in eine Richtung, Geschwindigkeit 2, SXCS Agenten)}
\label{max_stack_size_difficult:fig}
\end{figure}
