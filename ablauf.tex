...
Keine der bisher vorgestellten Varianten machen Gebrauch von einem sogenannten \emph{base reward}, d.h. 
...
TODO neue rewarderhebung...
TODO Rewarderhebung für normale Agenten irrelevant, evtl teilen
...
In der Schleife der Funktion zur Berechnung eines Experiments (Programm~\ref{mainExperiment:fig}) wird die Funktion zur Berechnung des Problems (\emph{doOneMultiStepProblem()}, Programm~\ref{mainProblem:fig}) aufgerufen. Dort wird, in einer weiteren Schleife über die Anzahl der maximalen Schritte, die Sicht aktualisiert (\emph{updateSight()}), die Qualität bestimmt (\emph{updateStatistics()}), die neuen Sensordaten und die nächste Aktion ermittelt (\emph{calculateAgents()}, siehe Programm~\ref{mainCalculate:fig}), der \emph{reward} Wert ermittelt (\emph{rewardAgents()}, siehe Programm~\ref{mainReward:fig}) und schließlich die Objekte bewegt (\emph{moveAgents()}, siehe Programm~\ref{mainMove:fig}). Die konkrete Umsetzung der dort aufgerufenen Funktionen (insbesondere \emph{calculateNextMove()} und \emph{calculateReward()}) wird im Kapitel~\ref{lcs_variants:cha} erläutert (bzw. in Kapitel~\ref{agents:cha} was die Heuristiken betrifft, wobei \emph{calculateReward()} dort keine Rolle spielt und eine leere Funktion aufgerufen wird).
...
\end{verbatim}
 \label{mainCalculate:fig}
  \caption{Zentrale Bearbeitung (Sensordaten und Berechnung der neuen Aktion) aller Agenten und des Zielobjekts innerhalb eines Problems}
\end{program}
...
\end{verbatim}
 \label{mainReward:fig}
  \caption{Zentrale Bearbeitung (Verteilung des Rewards) aller Agenten und des Zielobjekts innerhalb eines Problems}
\end{program}
...
\begin{program}
  ...
  \end{verbatim}
 \label{mainMove:fig}
  \caption{Zentrale Bearbeitung (Ausführung der Bewegung) aller Agenten und des Zielobjekts innerhalb eines Problems}
\end{program}
