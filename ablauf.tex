...
\section{Implementierung eines Problemablaufs}

Wie in den vorangegangenen Abschnitten beschrieben wird in einer großen Schleife (Programm~\ref{mainProblem:fig}) die Qualität bestimmt (``\emph{updateStatistics()}'') und dann die Sensordaten und die nächste Aktion ermittelt (``\emph{calculateAgents()}'', Programm~\ref{mainCalculate:fig}).

Die Implementierung der Berechnung eines Problems (Programm~\ref{mainCalculate:fig}) wird in einer Schleife von  aufgerufen, welches wiederum von der oben erwähnten Schleife~\ref{mainExperiment:fig} aufgerufen wird.
