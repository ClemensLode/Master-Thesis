...
In Kapitel\ref{stand_wissenschaft:cha} soll der gegenwärtige Stand der Forschung dargestellt werden, insbesondere in Bereichen, die sich mit dem Thema dieser Arbeit schneiden. In Kapitel~\ref{scenario_description:cha} soll dann eingehend auf das verwendete Szenario, die Eigenschaften der Objekte und vor allem die Eigenschaften der Agenten und des Zielobjekts eingegangen werden. Schließlich soll erläutert werden, wie die Simulation auf dem beschriebenen Szenario ablaufen soll. In Kapitel~\ref{lcs:cha} sollen dann die wichtigsten Teile des XCS vorgestellt werden, insbesondere die sogenannten \emph{classifier}, die Verarbeitung von Sensordaten, der allgemeine Ablauf und die XCS Parameter. Darauf aufbauend schließt Kapitel~\ref{lcs_variants:cha} an, in dem Anpassungen wie auch Verbesserungen des XCS Algorithmus besprochen werden sollen. Speziell für das vorgestellte Szenario soll desweiteren eine selbstentwickelte XCS Variante vorgestellt und dann durch die Erweiterung der Möglichkeit zur Kommunikation zwischen den Agenten weiterentwickelt werden. Der wesentliche Höhepunkt folgt dann in Kapitel~\ref{lcs_analysis:cha} in dem alle vorgestellten Algorithmen in den vorgestellten Szenarien getestet und analysiert werden. Abschluss bildet die Zusammenfassung und der Ausblick in Kapitel~\ref{conclusion:cha} und im Anhang in Kapitel~\ref{implementation:cha} findet sich dann noch eine Anzahl der zentralen, implementierten Quellcodes der Algorithmen, die in dieser Arbeit vorgestellt wurden.



\chapter{Stand der Wissenschaft}\label{stand_wissenschaft:cha}
