...
Zu einem wesentlich besseren Ergebnis könnte die Verwendung von einer größeren Anzahl von Sensoren bzw. realwertigen Eingabewerten führen. Das würde beispielsweise die Möglichkeit erlauben, den Abstand zu anderen Agenten je nach Szenario genauer zu regeln. Eine einführende Arbeit bezüglich realwertigen Eingabewerten und XCS findet sich z.B. in \cite{689040}. Mehrwertige Sensoren könnte man einfach durch Hinzunahme von weiteren Binärsensoren bewerkstelligen.
...
\subsection{Varianten zur Bestimmung des \emph{base reward}}

In Kapitel~\ref{bewertung:sec} wurde 

%if((sensor_goal[2*i])) {//  && (!sensor_agent[2*i+1])) { vergleichen. Ohne Sensoragent minimal schlechter
TODO, Bewertungskapitel
...
\caption[Screenshot des Konfigurationsprogramms] {Screenshot des Konfigurationsprogramms (Gesamtübersicht)}
...
\caption[Screenshot des Konfigurationsprogramms] {Screenshot des Konfigurationsprogramms (Konfigurationsbereich)}
