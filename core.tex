...
Könnte sich das Zielobjekt genauso langsam oder langsamer als die Agenten bewegen, dann wäre das Problem sehr simpel, da der Zielagent Schwierigkeiten hätte, Agenten abzuschütteln, deren einzige Strategie es ist, sich in Richtung des Zielagenten zu bewegen.
...
Jeder Agent besitzt 3 Gruppen mit jeweils 4 Binärsensoren. Alle Sensoren sind visuelle Sensoren mit begrenzter Reichweite. Andere Objekte blockieren die Sicht, Sichtlinien werden durch einen einfachen Bresenham-Linienalgorithmus bestimmt.\\
...
TODO Abbildung für den einfachen Fall, Darstellung der Sichtkegel, der Sichtweite und des Überwachungsradius
...
\caption[Einfacher Agent]{Sofern es sichtbar ist bewegt sich der Agent auf das Zielobjekt zu}
...
Der Code zur Generierung stammt aus \cite{Hamer}.
...
\item Falls \(A_{a} = \) NO\_ACTION : \[A_{a} = B_{a} = \mathrm{NO\_ACTION}\]
\item Falls \(A_{a} \neq \) NO\_ACTION : \[(A_{a}+i) \bmod 4 = B_{a}\]
\end{enumerate}
\end{enumerate}
...
\item Mit Hindernissen muss außerdem: \[(A_{y_{0}} A_{y_{1}} A_{y_{2}} A_{y_{3}}) = (B_{y_{0}} B_{y_{1}} B_{y_{2}} B_{y_{3}})\]
\end{enumerate}
...
\begin{program}
  \begin{verbatim}
     ...
     */
    public double checkDegreeOfRelationship(final MainClassifierSet other) {
        double degree = 0.0;
        int size = 0;
...
Der Kommunikationsaufwand ist hier nur minimal, neben dem Reward muss lediglich der Egoismus-Faktor übertragen werden. Auch muss der Faktor nur einmal pro Zeitschritt neuberechnet werden und es müssen nicht quadratisch viele Vergleiche mit anderen Agenten durchgeführt werden.
