...
Bei der Betrachtung der oben betrachteten Reihenfolge stellt sich die Frage, wann man die Qualität des Algorithmus messen sollte. Die Antwort hängt davon ab, was man denn nun eigentlich erreichen möchte. 
Der naheliegendste Messzeitpunkt ist, nachdem sich alle Agenten bewegt haben. Da die Agenten und das Zielobjekt in einem Durchlauf gemeinsam nacheinander bewegt werden, stellt sich die Frage nicht, ob womöglich vor der Bewegung des Zielobjekts die Qualität gemessen werden sollen.
...
\begin{enumerate}
\item Ermittlung der einzelnen \emph{base reward} Werte jeweils direkt nach der Ausführung einer einzelnen Aktion
\item Ermittlung aller \emph{base reward} Werte nach Ausführung aller Aktionen der Agenten und des Zielobjekts
\end{enumerate}
...
\item Bestimmung der jeweiligen {\bfseries {\em base reward} Werte} für die einzelnen Objekte für den letzten Schritt (für lernende Agenten)

\item Aktualisierung der Regeln anhand des \emph{base reward} Werts (für lernende Agenten)
